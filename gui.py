...
def get_att():
    v=int(e4.get())
    print(v)
    cur.execute("select name,percent,attendance from attendance where sno={}".format(v))
    for i in cur.fetchall():
        
        engine.say("Attendance percentage for {} bearing id{} is {} percent as he attended {} classes".format(i[0],v,i[1],i[2]))
        engine.runAndWait()
    connection.commit()
def reset():
    cur.execute("select sno from attendance")
